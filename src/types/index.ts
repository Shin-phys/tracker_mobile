...
export interface TrackedObject {
  ...
  /**
   * 最初に枠を引いたときの ROI。「やり直し」でここへ戻す。
   *
   * roi は追跡中に毎フレーム上書きされるので、追跡が終わった時点で
   * そこに入っているのは「物体が最後に到達した位置の枠」になる。
   * それを残したまま巻き戻して再生すると、物体のいない場所で
   * テンプレートが作り直され、その場で破綻する。
   * 取り直しを正しく成立させるには、引いた瞬間の枠を別に持っておく必要がある。
   */
  initialRoi: Rect | null;
  /**
   * その枠を引いたときのファイル上の時刻 [s]。
   * テンプレートは「引いた瞬間のコマの画」から作られるので、
   * 解析区間の始点がこの時刻とずれていると、始点のコマに物体がいない
   * ＝追跡が始まらない、ということが起きる。その確認にだけ使う。
   */
